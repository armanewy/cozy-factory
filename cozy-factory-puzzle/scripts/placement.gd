...
func _update_ghost() -> void:
	...
	if ghost_sprite.texture:
		var tw: float = float(ghost_sprite.texture.get_width())
		var th: float = float(ghost_sprite.texture.get_height())
		var target: float = float(grid.cell_size) * float(max(w, h))
		var s: float = min(target/tw, target/th) * 0.9
