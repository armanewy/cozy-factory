...
# Per-tile items: Dictionary(Vector2i => Array[Dictionary])
var tiles: = {}
var grid_cols := 12
var grid_rows := 8
...
func get_items(cell: Vector2i) -> Array:
    return tiles.get(_key(cell), [])

func set_items(cell: Vector2i, arr: Array) -> void:
    tiles[_key(cell)] = arr

func push_output(cell: Vector2i, kind: String, n:=1, dir:=Vector2i.ZERO) -> void:
    # Spawn items on a cell with zero progress towards dir
    if not in_bounds(cell):
        return
    var a := get_items(cell)
    for i in n:
...
func advance_items(cell: Vector2i, dir: Vector2i, dt_ms: int, tiles_per_sec: float) -> void:
    if not in_bounds(cell):
        return
    var a := get_items(cell)
    var speed := max(tiles_per_sec, 0.001)
    var delta_tiles := float(dt_ms) / 1000.0 * speed
    var remain: Array = []
    for it in a:
        var d := it
        d["dir"] = dir
